---
title: "How to Report T-Test Results in APA 7th Edition"
description: "Report t-test results in APA format with copy-ready examples for independent and paired samples. Includes Cohen's d, confidence intervals, and common formatting mistakes."
date: "2026-05-31"
category: "APA Reporting"
tags: ["t-test", "APA format", "statistical reporting", "effect size", "Cohen's d"]
---

## Quick Answer — Copy-Paste APA t-test Templates

**Independent-samples t-test:**

> An independent-samples *t*-test showed that [Group 1] (*M* = X.XX, *SD* = X.XX) scored significantly higher than [Group 2] (*M* = X.XX, *SD* = X.XX), *t*(df) = X.XX, *p* = .XXX, *d* = X.XX, 95% CI [X.XX, X.XX].

**Paired-samples t-test:**

> A paired-samples *t*-test indicated a significant change from [pre] (*M* = X.XX, *SD* = X.XX) to [post] (*M* = X.XX, *SD* = X.XX), *t*(df) = X.XX, *p* = .XXX, *d* = X.XX.

**One-sample t-test:**

> A one-sample *t*-test showed that [variable] (*M* = X.XX, *SD* = X.XX) differed significantly from the test value of X, *t*(df) = X.XX, *p* = .XXX, *d* = X.XX.

Rules: report *p* < .001 as `p < .001` (never `p = .000`); always include Cohen's *d* (APA 7th requires an effect size); italicize *t*, *p*, *M*, *SD*, *d*. Compute all of these instantly with the [StatMate t-test calculator](/calculators/t-test).
