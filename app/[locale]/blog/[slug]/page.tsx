...
const BLOG_CALCULATOR_MAP: Record<string, string> = {
  ...
  "fisher-exact-apa-reporting": "fisher-exact",
  "reliability-cronbach-alpha": "cronbach-alpha",
  "cronbach-alpha-apa-reporting": "cronbach-alpha",
...
const FAQ_DATA: Record<string, { question: string; answer: string }[]> = {
  ...
  "logistic-regression-apa-reporting": [
    ...
    { question: "What is the difference between the Wald test and the likelihood ratio test?", answer: "The Wald test evaluates individual predictor coefficients using the coefficient-to-standard-error ratio. The likelihood ratio test compares nested models by examining the change in -2 log likelihood. The likelihood ratio test is generally more reliable, especially for large coefficients where the Wald test can be conservative due to the Hauck-Donner effect." },
  ],
  ...
  "kruskal-wallis-apa-reporting": [
    { question: "What is the correct APA format for reporting a Kruskal-Wallis test?", answer: "The standard format is: H(df) = X.XX, p = .XXX, epsilon-squared = .XX. For example: H(2) = 18.42, p < .001, epsilon-squared = .31. Include the test justification, descriptive statistics with medians and IQRs, and post-hoc pairwise comparisons when the omnibus test is significant." },
    ...
    { question: "How do I calculate epsilon squared for a Kruskal-Wallis test?", answer: "Use the formula epsilon-squared = H / (N - 1), where H is the test statistic and N is the total sample size. Interpret the result using these benchmarks: .01 = small effect, .06 = medium effect, .14 = large effect. For example, with H = 18.42 and N = 60, epsilon-squared = 18.42 / 59 = .31 (large effect)." },
    { question: "When should I use Dunn's test vs pairwise Mann-Whitney U tests?", answer: "Dunn's test is preferred because it uses the same ranking from the original Kruskal-Wallis omnibus test, maintaining statistical consistency. Pairwise Mann-Whitney U tests re-rank the data for each comparison, which can lead to different rank orderings. Both require a correction for multiple comparisons (Bonferroni or Holm), but Dunn's test is the more methodologically sound choice." },
    { question: "What is the difference between Kruskal-Wallis and Friedman tests?", answer: "The Kruskal-Wallis test compares three or more independent groups (different participants in each group), while the Friedman test compares three or more related groups (the same participants measured under all conditions). The Kruskal-Wallis test is the nonparametric equivalent of the one-way ANOVA; the Friedman test is the nonparametric equivalent of repeated measures ANOVA." },
  ],
  "mann-whitney-apa-reporting": [
    { question: "Is Mann-Whitney U the same as the Wilcoxon rank-sum test?", answer: "Yes. The Mann-Whitney U test and the Wilcoxon rank-sum test are mathematically equivalent — they produce identical p-values and test the same null hypothesis. The naming difference is historical. Do not confuse the Wilcoxon rank-sum test (for independent groups) with the Wilcoxon signed-rank test (for paired samples)." },
    { question: "Should I report one-tailed or two-tailed p-values for Mann-Whitney U?", answer: "Use two-tailed p-values unless you specified a directional hypothesis before data collection. APA 7th edition recommends two-tailed tests as the default. If you use a one-tailed test, state this explicitly and justify why a directional prediction was warranted." },
    { question: "What is the minimum sample size for a Mann-Whitney U test?", answer: "The test can be performed with as few as 4 observations per group, but power will be very low. For adequate power (80%) to detect a medium effect (r = .30), aim for at least 20-30 observations per group. Use an a priori power analysis to determine the sample size needed for your specific research context." },
    { question: "Can I report both means and medians alongside the Mann-Whitney U result?", answer: "You may report means for additional context, but the primary descriptive statistics must be medians and interquartile ranges. If you include means, clarify that the Mann-Whitney U test does not evaluate mean differences and that the means are provided for descriptive completeness only." },
    { question: "How do I handle ties when reporting Mann-Whitney U results?", answer: "Most statistical software applies a tie correction to the z-score automatically. If ties are extensive (more than 15-20% of observations), mention the correction in your report. For small samples with many ties, prefer exact p-values over the asymptotic approximation, as ties affect the accuracy of the normal approximation more than the exact permutation distribution." },
  ],
  ...
  "factor-analysis-apa-reporting": [
    { question: "What is the minimum sample size for factor analysis?", answer: "There is no single correct answer. Guidelines range from a minimum of 100 observations to 10-20 participants per item. Comrey and Lee (1992) suggest N = 100 is poor, 200 is fair, 300 is good, 500 is very good, and 1000 is excellent. The stability of the factor solution also depends on communalities: with high communalities (above .60) and well-determined factors (3-4 items per factor), smaller samples may suffice." },
    { question: "Should I use principal component analysis (PCA) or factor analysis?", answer: "PCA and EFA are mathematically distinct. PCA reduces variables into composites without distinguishing shared variance from unique variance. EFA models latent constructs by analyzing only shared variance. For scale development and testing theoretical constructs, EFA (principal axis factoring or maximum likelihood) is preferred. PCA is acceptable for data reduction when latent constructs are not of theoretical interest." },
    { question: "What is the difference between the pattern matrix and the structure matrix?", answer: "The pattern matrix shows partial regression coefficients (each factor's unique contribution to each item, controlling for other factors). The structure matrix shows zero-order correlations (the total relationship between each item and factor, including shared variance from correlated factors). When factors are uncorrelated, the two matrices are identical. With oblique rotation, report the pattern matrix for interpretation." },
    { question: "How do I handle items that do not load on any factor?", answer: "Items with no loading above the threshold (e.g., .40) on any factor may have poor communalities, measure something outside the domain of interest, or be poorly worded. Remove these items and re-run the analysis. Report the removal and the rationale. After removing items, check that the remaining factor structure is stable and that KMO and variance explained have not deteriorated." },
    { question: "Can I conduct EFA and CFA on the same sample?", answer: "This practice is methodologically problematic because it is circular: you are confirming a structure on the data that generated it. The recommended approach is to split your sample randomly (requiring N > 400), conduct EFA on one half, and CFA on the other half. If your sample is too small to split, conduct EFA and recommend CFA in a future study with an independent sample." },
    { question: "How many items should load on each factor?", answer: "A minimum of three items per factor is generally required for a factor to be considered well-determined. Factors with only two items are unstable and may not replicate. Factors with four to six items are ideal, providing adequate reliability while keeping the scale parsimonious." },
    { question: "What does a negative factor loading mean?", answer: "A negative loading indicates that the item is inversely related to the factor. This is common with reverse-coded items. For example, in a factor measuring anxiety, the item 'I feel calm and relaxed' would be expected to load negatively. Check that the sign makes conceptual sense. If all items on a factor load negatively, the factor is simply reflected." },
    { question: "How do I report EFA results from SPSS?", answer: "In SPSS, run EFA via Analyze > Dimension Reduction > Factor. Report the KMO and Bartlett's test from the initial output, eigenvalues from the Total Variance Explained table, and loadings from the Rotated Factor Matrix (orthogonal) or Pattern Matrix (oblique). Suppress small loadings using the 'Suppress small coefficients' option set to .40. Communalities come from the Communalities table (Extraction column)." },
  ],
  "cohen-d-apa-reporting": [
    { question: "What is the difference between Cohen's d and Hedges' g?", answer: "Cohen's d and Hedges' g both measure the standardized mean difference between two groups. The key difference is that Hedges' g includes a small-sample correction factor that removes the slight upward bias inherent in Cohen's d. For samples with total N above 40, the two measures are nearly identical (the correction factor exceeds 0.99). For smaller samples, Hedges' g provides a less biased estimate. Most meta-analysis software uses Hedges' g by default." },
    { question: "Can Cohen's d be negative?", answer: "Yes. The sign of Cohen's d indicates the direction of the difference. A negative d means the second group's mean is higher than the first group's mean. In treatment studies, d is typically calculated as treatment minus control, so a positive d indicates the treatment group scored higher. Always specify which group is higher rather than relying solely on the sign." },
    { question: "What is a good Cohen's d value?", answer: "There is no universally good value. Cohen's benchmarks (0.20 small, 0.50 medium, 0.80 large) are rough guidelines, not standards. What matters is practical significance in context. In education, d = 0.40 may represent meaningful gains. In medicine, d = 0.20 for a life-saving intervention is highly important. Always compare your effect size to published benchmarks in your specific field." },
    { question: "How do I calculate Cohen's d from means and standard deviations?", answer: "For two independent groups: subtract the means (M1 - M2) and divide by the pooled standard deviation. The pooled SD equals the square root of [(SD1² + SD2²) / 2] for equal group sizes, or the square root of [((n1-1)SD1² + (n2-1)SD2²) / (n1+n2-2)] for unequal group sizes." },
    { question: "Should I report Cohen's d for non-significant results?", answer: "Yes, always. APA 7th edition requires effect sizes for all inferential tests regardless of statistical significance. Non-significant results with reported effect sizes and confidence intervals are valuable for meta-analyses, future power analyses, and distinguishing between true null effects and underpowered studies." },
    { question: "How does sample size affect Cohen's d?", answer: "Cohen's d itself is mathematically independent of sample size because it divides the mean difference by the standard deviation, not the standard error. However, sample size affects the precision of the estimate. Small samples produce highly variable d estimates with wide confidence intervals. Additionally, Cohen's d has a slight upward bias in small samples, which Hedges' g corrects." },
    { question: "Can I use Cohen's d for non-parametric tests?", answer: "Cohen's d is designed for comparing means and assumes approximately normal distributions. For non-parametric tests like Mann-Whitney U or Wilcoxon signed-rank, the appropriate effect size is the rank-biserial correlation r = Z / sqrt(N). You can approximate d from r using d = 2r / sqrt(1 - r²), but this conversion assumes normality and equal variances." },
    { question: "What software calculates Cohen's d with confidence intervals?", answer: "Most major statistical packages can compute Cohen's d: R (via effsize or MBESS packages), Python (via scipy or pingouin), JASP, and jamovi all provide d with 95% CIs. SPSS does not compute Cohen's d automatically and requires manual calculation or syntax. Online tools like StatMate's t-test calculator compute d, its 95% CI, and an APA-formatted results sentence automatically." },
  ],
  ...
  "fisher-exact-apa-reporting": [
    { question: "When should I use Fisher's exact test instead of chi-square?", answer: "Use Fisher's exact test when any expected cell frequency in your contingency table falls below 5, when the total sample size is below 20, or when any cell has an expected count of zero. These are situations where the chi-square approximation becomes unreliable. Some clinical journals recommend Fisher's exact test for all 2x2 tables regardless of sample size, since modern computing makes the exact calculation trivial." },
    { question: "Does Fisher's exact test produce a test statistic?", answer: "No. Unlike chi-square, ANOVA, or the t-test, Fisher's exact test does not generate a test statistic. It computes an exact p value directly from the hypergeometric distribution. When reporting in APA format, write 'Fisher's exact test, p = .XXX' — do not include a chi-square value." },
    { question: "What effect size should I report with Fisher's exact test?", answer: "For 2x2 tables, report the odds ratio (OR) with a 95% confidence interval. For tables larger than 2x2, report Cramer's V. If your study uses a prospective design (clinical trial, cohort study), you may additionally report the relative risk (RR). APA 7th edition requires an effect size for every inferential test." },
    { question: "Can Fisher's exact test be used for tables larger than 2x2?", answer: "Yes. The Freeman-Halton extension generalizes Fisher's exact test to any R x C contingency table. Most modern software supports this extension. For very large tables (beyond approximately 6x6), Monte Carlo simulation is used instead of exact enumeration. When reporting, note the table dimensions, the p value, the effect size (Cramer's V), and whether Monte Carlo simulation was used." },
    { question: "What is the difference between one-tailed and two-tailed Fisher's exact test?", answer: "The two-tailed test evaluates whether any association exists regardless of direction. The one-tailed test evaluates a specific directional hypothesis. The two-tailed p is always larger. Use two-tailed by default. Only use one-tailed if you stated a directional hypothesis before collecting data and can justify it based on prior research. Switching to one-tailed after seeing results inflates the Type I error rate." },
  ],
  "nonparametric-tests-guide": [
    { question: "Can I use nonparametric tests with large samples?", answer: "Yes, you can use nonparametric tests with any sample size. However, with large samples (n > 30 per group), the Central Limit Theorem often ensures that parametric test statistics are valid even with non-normal data. In such cases, parametric tests are generally preferred because they have slightly higher statistical power. The main exception is when your data are ordinal — nonparametric tests are appropriate regardless of sample size for ordinal data." },
    { question: "Is it acceptable to run both parametric and nonparametric tests and report whichever is significant?", answer: "No. This constitutes a form of p-hacking. You should decide which test to use based on your data characteristics and assumptions before examining the results. If you run both tests as a sensitivity analysis, report both results and note their agreement or disagreement. Do not selectively report only the test that produces a significant result." },
    { question: "How do I handle ties in nonparametric tests?", answer: "Ties (identical values) are common, especially with ordinal data. Most nonparametric tests handle ties by assigning the average of the ranks that the tied values would have occupied. For example, if two values are tied at positions 3 and 4, both receive a rank of 3.5. Modern statistical software handles ties automatically. When ties are extensive (more than 15–20% of the data), consider using a correction factor or reporting exact p-values rather than asymptotic approximations." },
    { question: "Should I report the z-approximation or the exact p-value for Mann-Whitney U?", answer: "For small samples (total N less than 40), exact p-values are preferred because the normal approximation may not be accurate. For larger samples, the z-approximation is standard and computationally practical. Many journals and reviewers prefer seeing the z-statistic reported alongside U because it facilitates effect size calculation (r = Z / sqrt(N)). Report both when possible: U = 45.00, z = -2.52, p = .012." },
    { question: "Can nonparametric tests detect interaction effects?", answer: "Standard nonparametric tests like the Kruskal-Wallis and Friedman tests are designed for one-factor designs and cannot directly test interaction effects. For factorial designs with interaction terms, there is no widely accepted nonparametric equivalent of two-way ANOVA. Options include the Scheirer-Ray-Hare test (which has limitations) or aligned rank transform (ART) ANOVA. In practice, many researchers use parametric ANOVA for factorial designs even with non-normal data, relying on the robustness of ANOVA." },
    { question: "What is the difference between the Mann-Whitney U test and the Wilcoxon rank-sum test?", answer: "They are the same test. The Mann-Whitney U test and the Wilcoxon rank-sum test are mathematically equivalent and always produce the same p-value. The difference is historical: Mann and Whitney developed one formulation, while Wilcoxon independently developed another. Do not confuse the Wilcoxon rank-sum test (for independent samples) with the Wilcoxon signed-rank test (for paired samples) — these are different tests." },
    { question: "When should I use Spearman's rho instead of Pearson's r?", answer: "Use Spearman's rho when one or both variables are ordinal, when the relationship between variables is monotonic but not linear, when there are significant outliers, or when the bivariate normality assumption is violated. If both variables are continuous, approximately normally distributed, and the relationship appears linear in a scatterplot, Pearson's r is preferred because it has greater statistical power." },
    { question: "How do I determine sample size for nonparametric tests?", answer: "Power analysis for nonparametric tests requires specifying the expected effect size, desired power (typically .80), and significance level (typically .05). Software such as G*Power can compute sample sizes for the Mann-Whitney U test, Wilcoxon signed-rank test, and other nonparametric tests. As a general rule of thumb, increase your parametric sample size estimate by 5–15% to account for the lower asymptotic relative efficiency of nonparametric tests." },
  ],
  "sample-size-determination": [
    { question: "How many participants do I need for a pilot study?", answer: "Pilot studies assess feasibility and provide preliminary effect size estimates, not statistical significance. Formal power analysis is generally not required. Common recommendations range from 12 per group (Julious, 2005) to 30 per group (Lancaster et al., 2004). The key is having enough participants to assess variability in your outcome measure." },
    { question: "Can I use a sample size calculator if my design has covariates?", answer: "Standard calculators assume simple designs without covariates. When you include covariates (e.g., ANCOVA), effective error variance decreases, so you need a smaller sample for the same power. A rough adjustment is to multiply the standard sample size by (1 - R²), where R² is the variance explained by covariates. For precise estimates, use simulation-based power analysis." },
    { question: "What is the minimum sample size for any statistical test?", answer: "There is no universal minimum. Required sample size depends entirely on effect size, significance level, and desired power. Practical minimums exist: for parametric tests, at least 15-20 per group for the central limit theorem; for chi-square tests, expected cell frequencies should be at least 5. These are necessary but not sufficient conditions for adequate power." },
    { question: "Should I use one-tailed or two-tailed tests for power analysis?", answer: "Use two-tailed tests unless you have a strong, pre-registered directional hypothesis and would genuinely not be interested in an effect in the opposite direction. One-tailed tests reduce required sample size by approximately 20%, but are heavily scrutinized by reviewers. When in doubt, plan for two-tailed." },
    { question: "Is it ethical to collect more data than the power analysis requires?", answer: "Generally yes, as long as additional data collection does not impose undue burden on participants. Extra participants provide more precise effect size estimates and increase power for secondary analyses. However, you must not use additional data to fish for significance — your primary analysis should follow the pre-registered plan." },
    { question: "What if I cannot recruit enough participants?", answer: "Options include: (1) use a more sensitive design such as within-subjects or ANCOVA; (2) accept lower target power (e.g., 0.70) while acknowledging this limitation; (3) focus on larger expected effect sizes by refining your intervention; (4) collaborate with other sites for multi-center recruitment. Do not simply proceed with an underpowered study without transparent disclosure." },
    { question: "Why is post-hoc power analysis considered inappropriate?", answer: "Post-hoc (observed) power is a direct mathematical transformation of the p-value and provides no additional information. If p = .05, observed power is approximately .50; if p = .001, observed power is approximately .95. Instead, report confidence intervals for the effect size and conduct sensitivity analyses for future studies." },
    { question: "How does Bayesian sample size planning differ from frequentist power analysis?", answer: "Bayesian approaches determine sample size based on the precision of the posterior distribution or the probability of reaching a decisive Bayes factor. Instead of targeting a fixed power level, you plan for a study where the probability of obtaining strong evidence (e.g., BF > 10) exceeds a threshold (e.g., 80%). Bayesian methods can incorporate prior information, potentially reducing sample size requirements." },
  ],
  "choosing-statistical-test": [
    { question: "Can I use a parametric test if my data are not perfectly normal?", answer: "Yes, in most cases. Parametric tests (t-test, ANOVA) are robust to moderate violations of normality, especially with equal sample sizes and n > 30 per group. The central limit theorem ensures sampling distributions of means approach normality. Switch to non-parametric alternatives only when samples are small and distributions are severely skewed or contain extreme outliers." },
    { question: "What is the difference between one-tailed and two-tailed tests?", answer: "A two-tailed test checks for effects in either direction, while a one-tailed test checks only one direction. Use two-tailed by default — it is the standard in most fields. Only use one-tailed when you have a strong theoretical reason to predict the direction AND would not report an effect in the opposite direction. One-tailed tests have more power but are heavily scrutinized." },
    { question: "When should I use MANOVA instead of multiple ANOVAs?", answer: "Use MANOVA when you have multiple correlated dependent variables and want to test whether groups differ on the combination of outcomes. Running separate ANOVAs inflates Type I error. However, MANOVA has additional assumptions (multivariate normality, homogeneity of covariance matrices) and is harder to interpret. If DVs are uncorrelated or you have a single primary outcome, separate ANOVAs with Bonferroni correction may be more appropriate." },
    { question: "How do I choose between chi-square and Fisher's exact test?", answer: "Use chi-square when expected cell frequencies meet the Cochran guideline: no more than 20% of cells below 5, none below 1. If conditions are not met, use Fisher's exact test. For 2×2 tables, many statisticians recommend always using Fisher's exact test. For larger tables, use chi-square when expected frequencies are adequate, or the Freeman-Halton extension otherwise." },
    { question: "What test should I use for ordinal data like Likert scales?", answer: "This is debated. Strictly, ordinal data violate parametric assumptions. Non-parametric tests are theoretically correct. However, simulation research shows t-tests and ANOVAs work well with 5+ point scales when samples are adequate and distributions not heavily skewed. Most researchers treat 5-7 point Likert scales as approximately interval, using parametric tests with non-parametric robustness checks." },
    { question: "Can I use regression instead of ANOVA?", answer: "Yes. ANOVA and regression are mathematically equivalent — ANOVA is a special case of regression with categorical predictors coded as dummy variables. Regression is more flexible: it handles continuous and categorical predictors simultaneously, tests interactions, and accommodates unequal group sizes naturally. Many statisticians recommend regression as the default framework." },
    { question: "What if my study has both between-subjects and within-subjects factors?", answer: "Use mixed ANOVA (split-plot ANOVA). This design has at least one between-subjects factor and one within-subjects factor. The key advantage is testing the interaction between them — for example, whether the change over time differs between treatment and control groups." },
    { question: "How do I handle violations of sphericity in repeated measures ANOVA?", answer: "Test sphericity using Mauchly's test. If violated, apply corrections: Greenhouse-Geisser when epsilon < 0.75, Huynh-Feldt when epsilon > 0.75. These adjust degrees of freedom downward, making the F test more conservative. Alternatively, use multivariate tests (Pillai's trace, Wilks' lambda) which do not assume sphericity." },
  ],
  "reliability-cronbach-alpha": [
    { question: "What is the minimum acceptable Cronbach's alpha?", answer: "The most widely cited minimum is .70 for research purposes (Nunnally & Bernstein, 1994). However, this threshold is context-dependent. In exploratory research or early-stage scale development, .60 may be acceptable. For clinical or high-stakes assessments where individual-level decisions are made, .90 is the recommended minimum. Always consider the number of items and the mean inter-item correlation alongside the alpha value." },
    { question: "What does it mean when alpha-if-item-deleted is higher than current alpha?", answer: "It means that removing that item would improve the scale's internal consistency. The item is introducing noise or measuring something different from the other items. However, do not remove items automatically. First check whether the item is reverse-scored correctly, whether it covers essential content, and whether the improvement in alpha is meaningful (e.g., more than .02)." },
    { question: "Can Cronbach's alpha be negative?", answer: "Yes, alpha can be negative when the inter-item covariances are predominantly negative. This typically occurs when items are not reverse-scored correctly, when items measure opposing constructs, or when the data contain extreme errors. A negative alpha always signals a data preparation problem rather than a legitimate reliability estimate." },
    { question: "Should I report alpha from the development sample or my study sample?", answer: "Always report the alpha obtained in your current sample. Alpha is a sample-dependent statistic, not a fixed property of a test. You may cite the development-sample alpha for comparison, but the primary value should come from your data. This is important because reliability varies with sample characteristics such as heterogeneity, cultural background, and age." },
    { question: "How is McDonald's omega different from Cronbach's alpha?", answer: "Both estimate internal consistency reliability, but they make different assumptions. Alpha assumes tau-equivalence, meaning all items contribute equally to the total score. Omega uses a factor-analytic model and allows items to have different factor loadings. When items have unequal loadings (which is almost always the case), alpha underestimates true reliability. Many methodologists now recommend reporting omega alongside or instead of alpha." },
    { question: "What is the relationship between reliability and validity?", answer: "Reliability is a necessary but not sufficient condition for validity. The maximum possible validity coefficient (correlation with a criterion) is the square root of the reliability coefficient. So a scale with alpha = .64 cannot correlate higher than .80 with any criterion. However, a highly reliable scale can still be invalid if it consistently measures the wrong construct." },
    { question: "How many items do I need for adequate Cronbach's alpha?", answer: "There is no universal answer because alpha depends on both the number of items and the strength of inter-item correlations. With mean inter-item correlations around .30, you need approximately 7-8 items to reach alpha = .75 and 12-14 items to reach alpha = .85. With stronger correlations (mean r = .50), 5 items suffice for alpha = .83. Use the Spearman-Brown prophecy formula to estimate the required number of items." },
    { question: "Should I use Cronbach's alpha or KR-20 for dichotomous items?", answer: "KR-20 and Cronbach's alpha are mathematically identical for dichotomous items and produce the exact same value. The choice is one of convention: using KR-20 signals to readers that your items are binary (correct/incorrect, yes/no). If your instrument uses mixed formats (some binary, some Likert), Cronbach's alpha is the appropriate choice for the overall scale." },
  ],
  "cronbach-alpha-apa-reporting": [
    { question: "What is the minimum acceptable Cronbach's alpha for APA reporting?", answer: "The most widely cited threshold is .70 for research purposes (Nunnally & Bernstein, 1994). However, the appropriate minimum depends on context. In exploratory or early-stage research, .60 may be acceptable. For clinical instruments used in individual-level decision-making, .90 is recommended. Always report the alpha value you obtained and let readers evaluate it in context, rather than omitting low values." },
    { question: "Can Cronbach's alpha be too high?", answer: "Yes. An alpha above .95 may indicate item redundancy, meaning multiple items ask essentially the same question in slightly different words. While this produces impressive reliability, it reduces content breadth and increases respondent burden. Examine the inter-item correlation matrix for clusters of very high correlations (above .80) and consider removing redundant items." },
    { question: "Should I report Cronbach's alpha in the Method or Results section?", answer: "APA convention places reliability information in the Measures subsection of the Method section. Report the alpha coefficient, number of items, and a verbal descriptor (e.g., good, acceptable) for each scale used. If reliability analysis is central to your research questions, such as in psychometric or scale development studies, additional detail in the Results section is appropriate." },
    { question: "Does Cronbach's alpha prove my scale is unidimensional?", answer: "No. This is a common misconception. Cronbach's alpha measures internal consistency but does not test dimensionality. A multidimensional scale with correlated subscales can produce a high alpha while measuring multiple constructs. To establish unidimensionality, conduct exploratory or confirmatory factor analysis before computing alpha." },
    { question: "How do I report alpha when using an established instrument I did not develop?", answer: "Report the alpha obtained from your own sample data and cite the original validation alpha for comparison. For example: The scale demonstrated good internal consistency in the present sample (alpha = .88, 21 items), consistent with values reported in the original validation (alpha = .91; Author, Year). Never report only the original validation alpha as if it applies to your data." },
  ],
  "normality-test-guide": [
    { question: "Which normality test should I use: Shapiro-Wilk or Kolmogorov-Smirnov?", answer: "For most research purposes, use the Shapiro-Wilk test. It has greater statistical power than the Kolmogorov-Smirnov test for sample sizes up to 2,000, meaning it is better at detecting genuine departures from normality. The K-S test (with Lilliefors correction) is an acceptable alternative only when your sample exceeds 2,000 observations or when the Shapiro-Wilk test is not available in your software." },
    { question: "What sample size do I need for a reliable normality test?", answer: "There is no minimum sample size for running a normality test, but statistical power increases with sample size. With fewer than 20 observations, normality tests have very low power and may fail to detect substantial non-normality. With 20-100 observations, normality tests are most informative. Above 100, tests become overly sensitive and should be supplemented with effect-size measures such as skewness and kurtosis values." },
    { question: "Should I test normality on raw data or on residuals?", answer: "It depends on the analysis. For t-tests and ANOVA, test normality within each group separately — the assumption is that the dependent variable is normally distributed within each group. For regression, the normality assumption applies to the residuals, not the raw predictor or outcome variables." },
    { question: "What if the Shapiro-Wilk test is significant but the Q-Q plot looks normal?", answer: "This discrepancy typically occurs with large samples, where the Shapiro-Wilk test detects trivial deviations that have no practical consequence. In such cases, the visual evidence from the Q-Q plot is more informative than the p-value. Report both results, explain that the departure is negligible in magnitude, and proceed with parametric tests." },
    { question: "Can I use normality tests with ordinal or Likert-scale data?", answer: "Technically yes, but their interpretation is questionable for ordinal or Likert-scale data. Discrete data with limited response options will almost always fail a normality test because the data cannot form a smooth, continuous distribution. Focus instead on skewness and kurtosis values, and consider whether the total score across multiple items is approximately normal." },
